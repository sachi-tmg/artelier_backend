...
const maxSize = 5 * 1024 * 1024; // 5MB for covers
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `COVER-${timestamp}-${randomBytes}${ext}`);
  },
});
...
const imageFileFilter = (req, file, cb) => {
  try {
    ...
    console.error('Cover upload filter error:', error);
...
const logSecurityEvent = async (req, action, reason, details) => {
  try {
    await AuditLogger.log({
      ...
      details: { reason, uploadType: 'cover', ...details }
    });
  } catch (error) {
  ...
  upload(req, res, async (err) => {
      ...
      if (err instanceof multer.MulterError) {
        switch (err.code) {
          ...
          case 'LIMIT_UNEXPECTED_FILE':
            return res.status(400).json({
              success: false,
              message: 'Unexpected field name. Use "coverPicture" as field name.',
              code: 'INVALID_FIELD'
            });
          default:
            return res.status(400).json({
    ...
    if (req.file) {
      try {
        ...
        console.error('Cover upload verification error:', error);
