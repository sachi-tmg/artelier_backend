...
const auditMiddleware = (options = {}) => {
  return async (req, res, next) => {
    ...
    // Skip noisy or passive endpoints
    const skipPaths = [
      '/health', '/ping', '/favicon.ico',
      '/api/creation/latest-creations',
      '/api/csrf/token'
    ];
    if (skipPaths.some(path => resource.startsWith(path))) return next();

    // Skip non-sensitive GETs (unless admin/sensitive)
    if (
      method === 'GET' &&
    ...
    res.on('finish', async () => {
      try {
        const duration = Date.now() - startTime;
        const user = req.user || null;
        const action = determineAction(resource, method, res.statusCode);

        // ⛔️ Skip log if action is generic/irrelevant
        if (!action || action === 'resource_accessed') return;
...
function determineAction(resource, method, statusCode) {
  ...
  // Avoid cluttering logs with generic GETs
  if (method === 'GET') return null;

  switch (method) {
    case 'POST': return 'resource_created';
    case 'PUT':
    case 'PATCH': return 'resource_updated';
    case 'DELETE': return 'resource_deleted';
    default: return null;
  }
}
...
function determineLogStatus(code) {
  if (code >= 200 && code < 300) return 'success';
  if (code >= 300 && code < 400) return 'info';
  if (code >= 400 && code < 500) return 'warning';
  if (code >= 500) return 'failure';
  return 'info';
}
...
function shouldLogRequestBody(resource, method) {
  ...
  const sensitiveEndpoints = [
    '/login', '/registerUser', '/update-password',
    '/reset-password', '/verify-mfa'
  ];

  if (sensitiveEndpoints.some(path => resource.includes(path))) return false;
  return method !== 'GET' && logBodyPaths.some(path => resource.includes(path));
}
