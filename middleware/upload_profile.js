...
const maxSize = 2 * 1024 * 1024; // Reduced to 2MB for security
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `PROFILE-${timestamp}-${randomBytes}${ext}`);
  },
});
...
const imageFileFilter = (req, file, cb) => {
  try {
    ...
    console.error('Profile upload filter error:', error);
...
const logSecurityEvent = async (req, action, reason, details) => {
  try {
    await AuditLogger.log({
      ...
      details: { reason, uploadType: 'profile', ...details }
    });
  } catch (error) {
...
const uploadsProf = (req, res, next) => {
  upload(req, res, async (err) => {
    ...
    if (req.file) {
      try {
        ...
        console.error('Profile upload verification error:', error);
